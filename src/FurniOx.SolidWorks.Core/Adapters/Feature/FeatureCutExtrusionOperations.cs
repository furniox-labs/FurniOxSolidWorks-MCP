...
public sealed class FeatureCutExtrusionOperations : OperationHandlerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // The sketch MUST be pre-selected before FeatureCut4.
            // UseAutoSelect only controls body selection, NOT sketch selection.
            var lastSketch = FindLastSketch(model);
            if (lastSketch == null)
            {
                return Task.FromResult(ExecutionResult.Failure("No sketch found to cut. Create a sketch first."));
            }
            ...
            if (!sketchSelected)
            {
                return Task.FromResult(ExecutionResult.Failure($"Failed to select sketch '{lastSketch.Name}' for cut extrusion."));
            }
