...
internal static class FeatureSupport
{
    ...
    public static string GetFilletOptionsDescription(int options)
    {
        ...
        if ((options & 0x2) != 0)
        {
            flags.Add("UniformRadius");
        }

        if ((options & 0x4) != 0)
        {
            flags.Add("StraightTransitions");
        }

        if ((options & 0x8) != 0)
        {
            flags.Add("UseHelpPoint");
        }

        if ((options & 0x20) != 0)
        {
            flags.Add("CornerType");
        }

        if ((options & 0x40) != 0)
        {
            flags.Add("AttachEdges");
        }

        if ((options & 0x80) != 0)
        {
            flags.Add("KeepFeatures");
        }

        if ((options & 0x100) != 0)
        {
            flags.Add("CurvatureContinuous");
        }

        if ((options & 0x200) != 0)
        {
            flags.Add("ConstantWidth");
        }

        if ((options & 0x400) != 0)
        {
            flags.Add("NoTrimNoAttach");
        }

        if ((options & 0x800) != 0)
        {
            flags.Add("ReverseFace1Dir");
        }

        if ((options & 0x1000) != 0)
        {
            flags.Add("ReverseFace2Dir");
        }

        if ((options & 0x2000) != 0)
        {
            flags.Add("PropagateFeatureToParts");
        }
